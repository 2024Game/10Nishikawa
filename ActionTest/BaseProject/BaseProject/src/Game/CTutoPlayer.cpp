...
// プレイヤーのインスタンス
CTutoPlayer* CTutoPlayer::spInstance = nullptr;

// プレイヤーのアニメーションデータのテーブル
const CTutoPlayer::AnimData CTutoPlayer::ANIM_DATA[] =
{
	{ "",						true,	0.0f,	1.0f	},	// Tポーズ
...
// コンストラクタ
CTutoPlayer::CTutoPlayer(CSaveManager* SaveManager)
...
CTutoPlayer* CTutoPlayer::Instance()
{
	return spInstance;
}

// アニメーション切り替え
void CTutoPlayer::ChangeAnimation(EAnimType type, bool restart)
{
...
// 状態を切り替え
void CTutoPlayer::ChangeState(EState state)
{
...
void CTutoPlayer::UpdateReserve()
{

}

// 待機
void CTutoPlayer::UpdateIdle()
{
	if (mHp > 0.0f)
	{
...
// 斬り攻撃
void CTutoPlayer::UpdateAttack1()
{
	switch (mStateStep)
	{
	case 0:
...
void CTutoPlayer::UpdateAttack2()
{
	switch (mStateStep)
	{
	case 0:
...
void CTutoPlayer::UpdateAttackX()
{
	switch (mStateStep)
	{
	case 0:
...
void CTutoPlayer::UpdateSlideAttack()
{
	switch (mStateStep)
	{
	case 0:
...
// ジャンプ開始
void CTutoPlayer::UpdateJumpStart()
{
...
// ジャンプ終了
void CTutoPlayer::UpdateJumpEnd()
{
	// ジャンプアニメーションが終了かつ、
	// 地面に接地したら、待機状態へ戻す
...
void CTutoPlayer::UpdateVictory()
{
	switch (mStateStep)
	{
	case 0:
...
// オブジェクト削除を伝える
void CTutoPlayer::DeleteObject(CObjectBase* obj)
{
...
// キーの入力情報から移動ベクトルを求める
CVector CTutoPlayer::CalcMoveVec() const
{
...
// モーションブラーの更新処理
void CTutoPlayer::UpdateMotionBlur()
{
...
void CTutoPlayer::LookCameraForward()
{
	// カメラの向きを取得（Y軸のみ）
	CCamera* camera = CCamera::MainCamera();
	if (camera)
...
void CTutoPlayer::LastUpdate()
{
	mpIndicator->SetShow(false);
	mInTypeAhead = false;
}

void CTutoPlayer::SetInBattle(int state)
{
	if (state == 0)
	{
		// 待機状態へ移行
...
// 攻撃中か
bool CTutoPlayer::IsAttacking() const
{
	// 斬り攻撃中
...
// ダメージを受ける
void CTutoPlayer::TakeDamage(float damage, CObjectBase* causer)
{
	// ベースクラスのダメージ処理を呼び出す
...
void CTutoPlayer::SetInvincible(bool invincible)
{
	if (!invincible)
	{
		// 当たり判定を通常のレイヤー設定にする
...
// 衝突処理
void CTutoPlayer::Collision(CCollider* self, CCollider* other, const CHitInfo& hit)
{
	// 本体のコライダーの衝突判定
	if (self == mpBodyCol)
	{
...
// 描画
void CTutoPlayer::Render()
{
	CXCharacter::Render();
}

float CTutoPlayer::GetMaxS1()
{
	return mS1RecastTime;
}

float CTutoPlayer::GetS1()
{
	return mS1CastTime;
}