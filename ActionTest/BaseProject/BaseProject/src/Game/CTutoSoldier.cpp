...
// コンストラクタ
CTutoSoldier::CTutoSoldier(CPlayer* player, int enemyLevel)
...
// 攻撃中か
bool CTutoSoldier::IsAttacking() const
{
	// 斬り1攻撃中
...
// ダメージを受ける
void CTutoSoldier::TakeDamage(float damage, CObjectBase* causer)
{
...
// 衝突処理
void CTutoSoldier::Collision(CCollider* self, CCollider* other, const CHitInfo& hit)
{
	// ベースの衝突処理を呼び出す
	...
	if (self == mpSword->Collider())
	{
		...
		if (hitChara != nullptr && !IsAttackHitObj(hitChara))
		{
			...
			{
				// 斬り攻撃1
			case (int)EState::eAttack1:		hitChara->TakeDamage(0 * mAttackMag, this);	break;
				// 斬り攻撃1B
			case (int)EState::eAttack1B:	hitChara->TakeDamage(0 * mAttackMag, this);	break;
				// 斬り攻撃2
			case (int)EState::eAttack2:		hitChara->TakeDamage(0 * mAttackMag, this);	break;
				// 斬り攻撃X
			case (int)EState::eAttackX:		hitChara->TakeDamage(0 * mAttackMag, this);	break;
				// 遠距離攻撃
	...
	else if (mpSlash != nullptr && self == mpSlash->Collider())
	{
		...
		{
			AddAttackHitObj(hitChara);
			hitChara->TakeDamage(0 * mAttackMag, this);
		}
	}
}

void CTutoSoldier::ChangeAnimation(int type, bool restart)
{
...
void CTutoSoldier::UpdateTactics()
{
...
void CTutoSoldier::UpdateBattleTempo()
{
...
// 距離も考慮して最終的な行動を返す
void CTutoSoldier::DecideNextAction()
{
...
// 戦闘相手の方へ向く
void CTutoSoldier::LookAtBattleTarget(bool immediate)
{
	// 戦闘相手がいなければ、処理しない
...
float CTutoSoldier::GetDistToTarget()
{
	// 現在地と目的地を取得
...
void CTutoSoldier::UpdateReserve()
{
}

void CTutoSoldier::UpdateTuto1()
{
...
// 斬り攻撃1の更新処理
void CTutoSoldier::UpdateAttack1()
{
	// ステップごとに処理を分ける
	switch (mStateStep)
	{
...
// 刈り取る右薙ぎ払い攻撃
void CTutoSoldier::UpdateAttack1B()
{
	// ステップごとに処理を分ける
	switch (mStateStep)
	{
...
// 斬り攻撃2の更新処理
void CTutoSoldier::UpdateAttack2()
{
	// ステップごとに処理を分ける
	switch (mStateStep)
	{
...
void CTutoSoldier::UpdateAttackX()
{
	switch (mStateStep)
	{
	case 0:
...
void CTutoSoldier::UpdateAvoidR()
{
	switch (mStateStep)
	{
	case 0:
...
void CTutoSoldier::UpdateAvoidL()
{
	switch (mStateStep)
	{
	case 0:
...
void CTutoSoldier::UpdateAvoidB()
{
	switch (mStateStep)
	{
	case 0:
...
void CTutoSoldier::UpdateVictory()
{
	switch (mStateStep)
	{
	case 0:
...
void CTutoSoldier::Update()
{
	...
	case EState::eReserve:	UpdateReserve();	break;
		// チュートリアル1状態
	case EState::eTuto1:	UpdateTuto1();		break;
		// チュートリアル2状態
	case EState::eTuto2:	UpdateTuto2();		break;
		// 待機状態
...
void CTutoSoldier::SetInBattle(int state)
{
	if (state == 0)
	{
		// 待機状態へ移行
