...
// 攻撃中か
bool CRainbowTrout::IsAttacking() const
{
	// パンチ攻撃中
...
// 攻撃開始
void CRainbowTrout::AttackStart()
{
	// ベースクラスの攻撃開始処理を呼び出し
	CEnemy::AttackStart();
...
// ダメージを受ける
void CRainbowTrout::TakeDamage(int damage, CObjectBase* causer)
{
	// ベースクラスのダメージ処理を呼び出す
...
// 衝突処理
void CRainbowTrout::Collision(CCollider* self, CCollider* other, const CHitInfo& hit)
{
	// ベースの衝突処理を呼び出す
...
// 戦闘相手の方へ向く
void CRainbowTrout::LookAtBattleTarget(bool immediate)
{
	// 戦闘相手がいなければ、処理しない
...
// 追いかける時の更新処理
void CRainbowTrout::UpdateChase()
{
	mMoveSpeed = CVector::zero;
...
// パンチ攻撃時の更新処理
void CRainbowTrout::UpdateAttack1()
{
	// ステップごとに処理を分ける
	switch (mStateStep)
	{
...
// 針攻撃時の更新処理
void CRainbowTrout::UpdateAttack2()
{
